...
function ready(s){const c=consultationOf(s);return c.status!=='completed'||Boolean(validDate(c.date)&&c.student_response.trim()&&c.agreed_direction.trim());}
function validate(s,{onlyKeys,fail,uuid,validateStrategy,validateActions}){
 ...
 if(s.preparation!==undefined&&s.preparation!==null){
  ...
  if(s.workflow_version!==2||!p.topic.trim()||!planFields.some(k=>p.strategy[k].trim())||Object.values(p.strategy).some(v=>v.includes('\0')))invalid();
 }
}
function requireReady(s,fail){
 if(!ready(s))fail(409,'CONSULTATION_REQUIRED','상담 완료로 표시하려면 상담 날짜·학생 반응·합의한 방향을 기록해 주세요.');
}
