...
async function readCase(actor,id) { if(!uuid(id))fail(404,'NOT_FOUND','상담 자료를 찾을 수 없습니다.');const rows=await db(`counseling_cases?id=eq.${id}&select=data,student_id&limit=1`);if(!rows?.length)fail(404,'NOT_FOUND','상담 자료를 찾을 수 없습니다.');await requireStudent(actor,rows[0].student_id);return rows[0].data; }
...
function sessionContent(s) { return {id:s.id,date:s.date,...Object.fromEntries(textFields.map(k=>[k,s[k]||''])),actions:s.actions||[]}; }
function hashSession(s) { return crypto.createHash('sha256').update(JSON.stringify(sessionContent(s))).digest('hex'); }
function validateSession(s) {
 onlyKeys(s,['id','date',...textFields,'actions','record','analysis','review','confirmed','imported_history']);
...
function freshSession() { return {id:newId(),date:now().slice(0,10),topic:'',student_question:'',context:'',evidence_notes:'',teacher_opinion:'',actions:[],next_date:'',record:null,analysis:null,review:null,confirmed:null}; }
...
function updateCase(stored,incoming) {
 ...
 incoming.sessions.forEach((s,i)=>{const old=stored.sessions[i];if(!old||old.id!==s.id)fail(400,'IMMUTABLE_HISTORY','기존 회차를 보존해 주세요.');for(const k of ['review','confirmed','imported_history'])if(JSON.stringify(s[k])!==JSON.stringify(old[k]))fail(400,'PROTECTED_REVIEW','검토·확정 정보는 해당 절차에서만 변경할 수 있습니다.');if(hashSession(old)!==hashSession(s)){if(old.confirmed)fail(409,'CONFIRMED_SESSION','확정 상담은 보존하고 다음 회차를 만들어 주세요.');out.sessions[i]={...structuredClone(s),review:null,confirmed:null};}});
 return out;
}
function reviewSession(s) {const notes=[];if(!s.topic.trim())notes.push('상담 주제를 작성해 주세요.');if(!s.teacher_opinion.trim())notes.push('교사의 상담 의견을 작성해 주세요.');if(!s.actions.some(a=>a.text.trim()))notes.push('다음 할 일을 한 가지 이상 작성해 주세요.');const incomplete=notes.length>0;const prose=textFields.filter(k=>k!=='next_date').map(k=>s[k]).join('\n');if(/(?:매우 중요|다양한|효과적으로|시사하는 바가 크)/.test(prose))notes.push('추상적인 표현은 확인한 행동이나 조건으로 다듬을 수 있는지 검토해 주세요. 직접 인용과 사실은 보존해 주세요.');notes.push('내용과 직접 인용을 보존하면서 문장별 핵심·구체성·반복 여부를 교사가 확인해 주세요.');return {state:incomplete?'needs_revision':'pending',method:'manual',content_hash:hashSession(s),notes,created_at:now()};}
...
function report(c,s) { const e=escapeHtml;return `<!doctype html><html lang="ko"><meta charset="utf-8"><title>대륜고 상담 기록</title><style>body{font-family:"Malgun Gothic",sans-serif;max-width:820px;margin:32px auto;line-height:1.7}p{white-space:pre-wrap;overflow-wrap:anywhere}h2{font-size:18px}section{break-inside:avoid}@page{size:A4;margin:18mm}@media print{button{display:none}}</style><button onclick="window.print()">인쇄 / PDF 저장</button><h1>대륜고 상담 기록 · ${s.confirmed?'확정':'초안'}</h1><p>${e(c.student.academic_year)}학년도 ${e(c.student.student_number)} ${e(c.student.name)} · ${e(s.date)}</p>${[['상담 주제',s.topic],['학생 질문',s.student_question],['상황',s.context],['확인한 근거',s.evidence_notes],['교사 의견',s.teacher_opinion],['다음 할 일',s.actions.map(a=>a.text+(a.due_date?` (${a.due_date})`:'')).join('\n')],['다음 상담',s.next_date]].map(([k,v])=>`<section><h2>${e(k)}</h2><p>${e(v)}</p></section>`).join('')}<p>상담 버전 ${c.revision} · ${e(c.teacher.display_name)} · ${s.confirmed?'교사 확인 '+e(s.confirmed.at):'교사 최종 확인 전'}</p></html>`; }
module.exports={HttpError,fail,uuid,newId,now,json,wrap,body,onlyKeys,db,identity,profileFor,auth,studentsFor,requireStudent,readCase,rejectPrivate,hashSession,validateCase,freshSession,newCase,revision,getSession,updateCase,reviewSession,writeCase,report};
