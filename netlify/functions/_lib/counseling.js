...
function normalizeCase(c) {const copy=structuredClone(c);copy.sessions=copy.sessions.map(s=>({...s,strategy:strategyOf(s),profile:P.profileOf(s),guidance:s.guidance??null}));return copy;}
function sessionContent(s) {const strategy=strategyOf(s),profile=P.profileOf(s);return {id:s.id,date:s.date,...Object.fromEntries(textFields.map(k=>[k,s[k]||''])),actions:s.actions||[],...(Object.values(strategy).some(Boolean)?{strategy}:{}),...(P.hasData(profile)?{profile}:{})}; }
...
function validateSession(s) {
 onlyKeys(s,['id','date',...textFields,'strategy','profile','guidance','actions','record','analysis','review','confirmed','imported_history']);
...
function freshSession() { return {id:newId(),date:now().slice(0,10),topic:'',student_question:'',context:'',evidence_notes:'',teacher_opinion:'',strategy:strategyOf({}),profile:P.profileOf({}),guidance:null,actions:[],next_date:'',record:null,analysis:null,review:null,confirmed:null}; }
