...
exports.handler=C.wrap(async event=>{
 ...
 if(event.httpMethod==='GET'){
  ...
  if(action==='report'){const s=C.getSession(c,q.session_id||c.current_session_id);return {statusCode:200,headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store','Content-Security-Policy':"default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; base-uri 'none'; frame-ancestors 'none'",'X-Content-Type-Options':'nosniff'},body:C.report(c,s)};}
 ...
 if(event.httpMethod==='POST'&&action==='import'){
  ...
  const c=C.newCase(student,actor);c.sessions=b.bundle.case.sessions.map(s=>({...structuredClone(s),id:C.newId(),record:null,analysis:null,review:null,confirmed:null,imported_history:{source_session_id:s.id,review:s.review,confirmed:s.confirmed}}));c.current_session_id=c.sessions.find(s=>s.imported_history.source_session_id===b.bundle.case.current_session_id).id;c.origin='import';c.imported_from={id:b.bundle.case.id,revision:b.bundle.case.revision,at:C.now()};return C.json(201,{case:await C.writeCase(actor,c,0,'import')});
 }
 ...
 if(event.httpMethod==='PUT'){if(action)C.fail(400,'UNKNOWN_ACTION','지원하지 않는 요청입니다.');C.onlyKeys(b,['case']);c=C.updateCase(stored,b.case);}
  ...
  C.onlyKeys(b,action==='confirm'?['revision','session_id','review_acknowledged']:action==='review'?['revision','session_id']:['revision']);C.revision(stored,b);
  if(action==='next'){const s=C.freshSession();c.sessions.push(s);c.current_session_id=s.id;}
