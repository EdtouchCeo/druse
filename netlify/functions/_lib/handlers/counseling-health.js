...
exports.handler=C.wrap(async event=>{
 ...
 return C.json(ready&&auth?200:503,{service:'daeryun-counseling',version:'0.5.3',storage:storage.mode(),storage_ready:ready,storage_diagnostic:ready?'READY':storage.diagnostic(checks[0].reason),school_auth_ready:auth,record_analysis:'local-ollama-only',server_ai_configured:getCounselingAiConfig().enabled});
});
