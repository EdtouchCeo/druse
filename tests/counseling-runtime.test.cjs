...
test('native health entry preserves automatic strong context and exposes no profile data',async()=>{
 ...
 assert.equal(response.status,200);const value=await response.json();assert.equal(value.storage_ready,true);assert.equal(value.school_auth_ready,true);assert.equal(value.server_ai_configured,false);
