...
test('student PDF preview preserves confirmed status and Korean strategy labels',async()=>{
 ...
 for(const label of ['대륜고 학종 전략 안내',c.sessions[0].topic,'목표 전공·관심 분야','진학 방향','강점','보완할 점','교과 학습 계획','탐구 계획','활동 계획','학기별 실행 계획','학생에게 전하는 안내','이번 실행 과제','인쇄 / PDF 저장'])assert.ok(response.body.includes(label),label);
 assert.ok(response.body.includes('<h2>다음 점검</h2><p>미정</p>'),'unknown next check is explicit without inventing a date');
...
test('AI receives stored strategy context without publication metadata and leaves records unchanged',async()=>{
 const c=legacySample(),m=mock([c]);process.env.COUNSELING_SERVER_AI_ENABLED='true';process.env.GEMINI_API_KEY='synthetic-only';let sent;const handler=createAi(async args=>{sent=args;return {ok:true,data:{candidates:[{content:{parts:[{text:'합성 전략 초안'}]}}]}};});
