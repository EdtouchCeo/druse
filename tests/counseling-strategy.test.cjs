...
test('legacy sessions gain empty strategy and null guidance without invalidating old content hashes',()=>{
 const c=legacySample(),s=c.sessions[0];delete s.strategy;delete s.guidance;
...
test('every strategy field changes the review hash while guidance never does',()=>{
 const s=legacySample().sessions[0],original=C.hashSession(s);
...
test('strategy schema rejects extra fields, nonstrings and incomplete objects',()=>{
 for(const mutate of [s=>{s.strategy.extra='private'},s=>{s.strategy.strengths=[]},s=>{delete s.strategy.gaps},s=>{s.strategy=null}]){const c=legacySample();mutate(c.sessions[0]);assert.throws(()=>C.validateCase(c),C.HttpError);}
});
test('strategy review uses student guidance and actions while retaining internal-only legacy review',()=>{
 const s=legacySample().sessions[0];s.teacher_opinion='';assert.equal(C.reviewSession(s).state,'pending');s.strategy.student_message='';assert.equal(C.reviewSession(s).state,'needs_revision');s.strategy=C.strategyOf({});s.teacher_opinion='기존 수기 의견';assert.equal(C.reviewSession(s).state,'pending');
});
test('draft strategy edits clear review and confirmed strategy edits are rejected',async()=>{
 const c=legacySample();c.sessions[0].review=C.reviewSession(c.sessions[0]);let m=mock([c]);const incoming=structuredClone(c);incoming.sessions[0].strategy.semester_plan='다음 학기에 점검';
...
test('guidance cannot be forged, cleared or replaced with a direct case update',async()=>{
 for(const source of [legacySample(),published()]){const incoming=structuredClone(source);incoming.sessions[0].guidance=source.sessions[0].guidance?null:{published_at:C.now(),published_by:ids.teacher};const m=mock([source]);const response=await cases(event('PUT',{case:incoming},{id:source.id}));assert.equal(response.statusCode,400);assert.equal(m.writes().length,0);}
});
...
test('publish needs current confirmation hash, student message and concrete strategy and is not repeated',async()=>{
 for(const c of [legacySample(),(()=>{const x=confirmed();x.sessions[0].strategy.gaps+=' changed';return x;})(),(()=>{const x=legacySample();x.sessions[0].strategy.student_message='';return confirmed(x);})(),(()=>{const x=legacySample();for(const field of C.planFields)x.sessions[0].strategy[field]='';return confirmed(x);})(),published()]){const m=mock([c]);const response=await cases(event('POST',{revision:c.revision,session_id:c.current_session_id},{id:c.id,action:'publish'}));assert.equal(response.statusCode,409);assert.equal(m.writes().length,0);}
});
...
test('student list/detail select only published sessions and remove internal and import metadata',async()=>{
 ...
 const hidden=legacySample();hidden.sessions[0].topic='PRIVATE_WHOLE_CASE';mock([c,hidden],{studentActor:true});
...
test('student exports and any requested report audience contain only guidance and reject draft IDs',async()=>{
 ...
 for(const audience of ['teacher','student']){const response=await cases(event('GET',undefined,{id:c.id,action:'report',audience}));assert.equal(response.statusCode,200);assert.ok(response.body.includes('학습·진로 전략 보고서'));assert.ok(!response.body.includes('PRIVATE_'));assert.ok(!response.body.includes('교사 내부 참고'));}
...
test('teacher can preview an unpublished student PDF without exposing internal notes or publishing',async()=>{
 const c=legacySample(),m=mock([c]);const preview=await cases(event('GET',undefined,{id:c.id,action:'report',audience:'student'}));assert.equal(preview.statusCode,200);assert.ok(preview.body.includes('초안'));assert.ok(preview.body.includes('물리학'));assert.ok(!preview.body.includes('PRIVATE_'));
...
test('student PDF preview preserves confirmed status and Korean strategy labels',async()=>{
 const c=confirmed();mock([c]);const response=await cases(event('GET',undefined,{id:c.id,action:'report',audience:'student'}));assert.equal(response.statusCode,200);assert.ok(!response.body.includes('검토용 초안'));assert.ok(!response.body.includes('초안'));assert.ok(!response.body.includes('PRIVATE_CONFIRM_METADATA'));
 for(const label of ['학습·진로 전략 보고서',c.sessions[0].topic,'목표 전공·관심 분야','진학 방향','강점','보완할 점','교과 학습 계획','탐구 계획','창체·봉사·독서·행동특성 전략','학년·학기별 성장 전략','학생에게 전하는 안내','인쇄 / PDF 저장'])assert.ok(response.body.includes(label),label);
 assert.ok(!response.body.includes('측정 조건표 작성'),'legacy actions stay in backups rather than current strategy reports');
 assert.ok(!response.body.includes('다음 점검'),'task tracking is excluded from consumer documents');
...
test('next strategy revision copies strategy without carrying legacy task tracking or publication',async()=>{
 const c=published();c.sessions[0].actions.push({id:C.newId(),text:'완료 과제',status:'done',due_date:''},{id:C.newId(),text:'진행 과제',status:'in_progress',due_date:''},{id:C.newId(),text:'보류 과제',status:'deferred',due_date:''});mock([c]);const response=await cases(event('POST',{revision:1},{id:c.id,action:'next'}));assert.equal(response.statusCode,200);const next=parsed(response).case.sessions[1];assert.deepEqual(next.strategy,c.sessions[0].strategy);assert.deepEqual(next.actions,[]);assert.equal(next.next_date,'');assert.deepEqual(next.profile,c.sessions[0].profile);assert.equal(next.guidance,null);assert.equal(next.confirmed,null);assert.equal(next.review,null);assert.equal(next.topic,'학습·진로 전략 이어가기');assert.equal(next.context,`이전 작성일: ${c.sessions[0].date}\n이전 전략 제목: ${c.sessions[0].topic}`);assert.deepEqual(parsed(response).case.sessions[0],c.sessions[0]);
});
test('retired AI blocks AI receives stored strategy context without publication metadata and leaves records unchanged',async()=>{
