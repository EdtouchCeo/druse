...
function sample(){const c=C.newCase(student,{id:ids.teacher,display_name:'합성 교사'}),s=c.sessions[0];s.topic='학습 자료 점검';s.teacher_opinion='자료를 확인하고 다음 질문을 정한다.';s.strategy.student_message='다음 시간에 학습 계획을 함께 점검합니다.';s.actions=[{id:C.newId(),text:'학습 계획 작성',status:'planned',due_date:''}];s.profile={...C.profileOf({}),interests:'PRIVATE_PROFILE_INTEREST',teacher_observations:'PRIVATE_PROFILE_OBSERVATION',selected_subjects:['PRIVATE_SELECTED_SUBJECT'],weekly_minutes:240,grades:[row()]};return c;}
...
function mock(c=sample(),studentMode=false){let stored=structuredClone(c);const calls=[];global.fetch=async(input,options={})=>{const url=new URL(input);calls.push({url,options});let data;
...
test('all profile fields participate in the hash and zero minutes is retained',()=>{
 const s=sample().sessions[0];s.profile=C.profileOf({});const empty=C.hashSession(s);
...
test('profile trims author input and rejects empty subjects, normalized duplicates and NUL characters',()=>{
 ...
 for(const mutate of mutations){const copy=sample();mutate(copy.sessions[0].profile);assert.throws(()=>C.validateCase(copy),C.HttpError);}
});
test('unfinished grade subject survives backup import and is clearly labeled in teacher PDF',async()=>{
 const c=sample();c.sessions[0].profile.grades[0].subject=' ';C.validateCase(c);const m=mock(c);
...
test('grade scales preserve unknown and achievement nulls and enforce numeric boundaries',()=>{
 ...
 for(const change of accepted){const c=sample();Object.assign(c.sessions[0].profile.grades[0],change);C.validateCase(c);}
 ...
 for(const change of invalid){const c=sample();Object.assign(c.sessions[0].profile.grades[0],change);assert.throws(()=>C.validateCase(c),C.HttpError,JSON.stringify(change));}
});
test('duplicate grade IDs and unexpected grade fields cannot enter a backup',()=>{
 const c=sample();c.sessions[0].profile.grades.push(structuredClone(c.sessions[0].profile.grades[0]));assert.throws(()=>C.validateCase(c),C.HttpError);c.sessions[0].profile.grades.pop();c.sessions[0].profile.grades[0].comment='unexpected';assert.throws(()=>C.validateCase(c),C.HttpError);
});
test('profile edits invalidate review and cannot modify a confirmed session',async()=>{
 const c=sample();c.sessions[0].review=C.reviewSession(c.sessions[0]);mock(c);const incoming=structuredClone(c);incoming.sessions[0].profile.weekly_minutes=300;
 ...
 const fixed=publish(sample()),m=mock(fixed),changed=structuredClone(fixed);changed.sessions[0].profile.grades[0].score=90;assert.equal((await cases(event('PUT',{case:changed},{id:fixed.id}))).statusCode,409);assert.equal(m.writes().length,0);
});
test('student list, detail, export and reports never contain profile keys or private values',async()=>{
 const c=publish(sample());mock(c,true);
...
test('teacher PDF includes escaped profile summary and grades while student preview omits both',async()=>{
 const c=sample();c.sessions[0].profile.teacher_observations='<script>PRIVATE_PROFILE_XSS</script>';mock(c);
...
test('invalid profile import fails before any storage write',async()=>{
 const c=sample();c.sessions[0].profile.grades[0].grade_scale='unknown';const m=mock();const response=await cases(event('POST',{bundle:{format:'daeryun-counseling',version:1,case:c},student_id:ids.student,student_confirmed:true},{action:'import'}));assert.equal(response.statusCode,400);assert.equal(m.writes().length,0);
});
test('teacher AI receives stored profile and evidence-first instructions without changing data',async()=>{
 const c=sample(),m=mock(c);process.env.COUNSELING_SERVER_AI_ENABLED='true';process.env.GEMINI_API_KEY='synthetic-only';let prompt;
