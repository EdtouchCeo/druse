...
export function isRevisionConflict(error:unknown):boolean{return error instanceof ApiError&&error.status===409&&error.code.toLowerCase()==='revision_conflict'}
export async function checked(response:Response):Promise<Response> {
 ...
 if(response.status===409&&code.toLowerCase()==='revision_conflict')message='다른 창에서 이 전략을 수정했습니다. 작성 내용을 백업한 뒤 최신 기록을 불러와 주세요.'
