...
export class CloudTransport implements Transport {
 ...
 async generalAi(value:CounselingCase,sessionId:string,settings:AiSettings,signal?:AbortSignal):Promise<string>{
  ...
  const prompt='다음 일반 상담과 학종 전략을 바탕으로 교사가 검토할 교과·탐구·활동·학기별 계획, 학생 안내와 실행과제를 제안하라. 목표 전공이 미정이면 탐색 대안을 제안하라. 학생이 실제로 하지 않은 일과 학생부 내용을 만들거나 합격을 보장하지 마라. 관찰 → 추가 확인 자료 → 실제 교과 연결 → 학생에게 확인할 질문 → 실행과제 순서로 제안하라. 미입력 자료는 학생의 약점으로 판단하지 마라. 5등급제와 9등급제를 혼합·환산하거나 평균 등급과 합격 가능성을 계산하지 마라. 관찰값, 해석, 앞으로의 계획을 구분하고 간결한 한국어로 작성하라.\n'+JSON.stringify({goal:session.topic,profile:session.profile,strategy:session.strategy,question:session.student_question,context:session.context,evidence_notes:session.evidence_notes,teacher_opinion:session.teacher_opinion,actions:session.actions.map(a=>({text:a.text,status:a.status}))})
