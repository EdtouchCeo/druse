...
export class LocalTransport implements Transport {
 ...
 async analyze(value:CounselingCase,sessionId:string,model:string,goal:string,_legacyBudget?:number){return(await this.request<{job:Job}>('/cases/'+encodeURIComponent(value.id)+'/analyze','POST',{revision:value.revision,session_id:sessionId,model,goal})).job}
 async refineRecord(value:CounselingCase,sessionId:string,model:string){return(await this.request<{job:Job}>('/cases/'+encodeURIComponent(value.id)+'/refine-record','POST',{revision:value.revision,session_id:sessionId,model})).job}
 async generateStrategy(value:CounselingCase,sessionId:string,model:string){return(await this.request<{job:Job}>('/cases/'+encodeURIComponent(value.id)+'/strategy-draft','POST',{revision:value.revision,session_id:sessionId,model})).job}
 async preparationStatus(id:string,sessionId:string){return this.request<{source_hash:string}>('/cases/'+encodeURIComponent(id)+'/preparation-status?session_id='+encodeURIComponent(sessionId))}
 async generatePreparation(value:CounselingCase,sessionId:string,stage:'admissions'|'inquiry',targetId:string){return(await this.request<{job:Job}>('/cases/'+encodeURIComponent(value.id)+'/preparation-strategy','POST',{revision:value.revision,session_id:sessionId,stage,target_id:targetId})).job}
 preparationPdf(id:string,sessionId:string,kind:'admissions'|'inquiry',targetId:string){return this.blob('/cases/'+encodeURIComponent(id)+'/preparation.pdf?session_id='+encodeURIComponent(sessionId)+'&kind='+kind+'&target_id='+encodeURIComponent(targetId))}
