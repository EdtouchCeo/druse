...
<template>
 <section class="card strategy-editor">
  <div class="section-heading"><span class="step">01</span><div><h2>{{phase==='published'?'나의 학종 전략':phase==='final'?'상담을 반영한 최종 전략':'상담 전 교사 전략'}}</h2><p>{{phase==='published'?'선생님이 안내한 방향과 계획을 읽고 아래 실행과제를 확인하세요.':phase==='final'?(readOnly?'확정한 전략입니다. 변경할 내용은 새 회차에서 작성하세요.':'학생과 합의한 방향에 맞춰 최종 계획과 안내 메시지를 수정합니다.'):'교사가 판단한 방향과 교과·탐구·활동 계획을 먼저 세웁니다. 학생 안내 메시지는 상담 후 완성할 수 있습니다.'}}</p></div></div>
