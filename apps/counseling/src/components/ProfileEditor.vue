...
<template>
 <div class="profile-form">
  ...
  <fieldset :disabled="locked">
   ...
   <div class="profile-input-grid"><label>실제 이수 과목<textarea v-model="subjectText" aria-label="실제 이수 과목" rows="2" placeholder="예: 공통국어1, 공통수학1 (쉼표로 구분)" @input="subjects"></textarea></label><label>주간 가용 시간 <small>분</small><input :value="profile.weekly_minutes" aria-label="주간 가용 시간" type="number" min="0" max="2400" step="1" placeholder="예: 60" @input="profile.weekly_minutes=numeric($event)"><small>빈칸: 미확인 · 0: 추가 시간 없음</small></label></div>
