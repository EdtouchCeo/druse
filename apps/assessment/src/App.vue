...
const completion = computed(() => guides.value.filter(g => g.key === 'ideas' && kind.value === 'social' ? getIdeaText(project.value).trim() : getField(project.value, `${kind.value === 'social' ? 'social' : 'canvas'}:${g.key}`).trim()).length)
...
function writeSocial(key: SocialKey, value: string) { project.value.social[key] = value; touchSocial(project.value) }
...
async function applyPracticeHere() {
  const practice = pendingPractice.value
  if (!practice) return
  if (project.value.social.situation.trim()) { announce('현재 원고에 문제 상황이 있어 새 원고로 시작해 주세요.'); return }
  project.value.technique = practice.technique; project.value.practiceId = practice.id; project.value.source = practice.source; project.value.social.situation = practice.situation; project.value.sources = [project.value.sources, practice.source].filter(Boolean).join('\n'); touchSocial(project.value)
  pendingPractice.value = null; dialog.value = ''; exampleMode.value = false; landing.value = false; step.value = 0; updateUrl(); announce('연습 문제를 가져왔습니다. 나머지 항목은 직접 작성하세요.')
}
...
function openVariation() { variationText.value = selectedPractice.value?.change || PRACTICES.find(p => p.technique === project.value.technique)?.change || ''; dialog.value = 'variation' }
...
<template>
    ...
    <template v-else-if="!loading">
      ...
      <div class="workspace-layout" :class="{ 'without-help': !helpOpen }">
        <aside class="steps-sidebar" :class="{ 'mobile-expanded': mobileSteps }">
          <button class="mobile-step-toggle" :aria-expanded="mobileSteps" @click="mobileSteps = !mobileSteps"><Menu :size="18" /> {{ String(step + 1).padStart(2, '0') }} {{ guide.label }}<span>{{ step + 1 }} / {{ guides.length }}</span></button>
          <div class="sidebar-content"><div class="sidebar-heading"><span>작성 순서</span><span>{{ completion }} / {{ guides.length }}</span></div><div class="progress-track"><span :style="{ width: `${completion / guides.length * 100}%` }" /></div><p class="progress-caption">내용을 쓴 항목 수입니다.</p>
          <nav aria-label="작성 단계"><button v-for="(g, index) in guides" :key="g.key" class="step-item" :class="{ active: index === step }" :aria-current="index === step ? 'step' : undefined" @click="setStep(index)"><span class="step-number"><Check v-if="isWritten(g) && index !== step" :size="15" /><template v-else>{{ String(index + 1).padStart(2, '0') }}</template></span><span>{{ g.label }}</span><ChevronRight v-if="index === step" :size="15" /></button></nav>
          <div class="sidebar-bottom"><p>생각을 이어 가세요.</p><button class="text-button" @click="navigate(kind === 'social' ? 'business' : 'social')">{{ kind === 'social' ? '이 계획으로 비즈니스 모델 작성' : '사회 문제 해결 계획서로' }}<ArrowRight :size="16" /></button><a href="/#student/life"><ArrowLeft :size="14" /> 학교생활로 돌아가기</a></div></div>
        </aside>
        <main id="workspace-main" class="editor-main">
          ...
          <div v-else-if="project.variation" class="variation-banner"><span class="eyebrow">조건 바꾸기 연습</span><p>{{ project.variation }}</p><small>바뀐 조건이 어느 단계에 영향을 주는지 찾아 고쳐 보세요. 수정 이유는 내 글 점검에 남길 수 있습니다.</small></div>
          ...
          <div class="editor-section-heading"><div><span class="eyebrow">STEP {{ String(step + 1).padStart(2, '0') }}</span><h2 id="editor-heading" tabindex="-1">{{ guide.label }}</h2></div><button class="help-toggle secondary" :aria-expanded="helpOpen" @click="helpOpen = !helpOpen"><Lightbulb :size="16" />{{ helpOpen ? '도움 접기' : '작성 도움' }}</button></div>
          ...
          <template v-if="exampleMode">
            ...
            <article class="example-document"><span class="eyebrow">{{ example.title }}</span><h3>{{ guide.label }}</h3><p>{{ kind === 'social' ? example.social[guide.key as SocialKey] : example.canvas[guide.key as CanvasKey] }}</p><footer>자료 출처: {{ example.source }}</footer></article>
            ...
            <div class="example-next"><p>이 예제와 내 문제는 무엇이 같고 무엇이 다른가요?</p><button class="primary" @click="setMode('guided')">내 상황으로 써 보기<ArrowRight :size="16" /></button></div>
          </template>

          <template v-else>
            <template v-if="kind === 'social' && guide.key === 'situation'">
              <div class="start-toolbar"><p>주어진 상황을 읽고 핵심 사실을 적어 보세요.</p><button class="secondary" @click="dialog = 'start'">연습 문제 고르기<ChevronRight :size="16" /></button></div>
              <div v-if="selectedPractice" class="practice-source"><span class="eyebrow">선택한 연습 문제</span><strong>{{ selectedPractice.title }}</strong><p>{{ selectedPractice.situation }}</p><small>{{ selectedPractice.source }}</small></div>
            </template>
          ...
          <footer class="editor-navigation"><button class="secondary" :disabled="step === 0" @click="setStep(step - 1)"><ArrowLeft :size="17" />이전 항목</button><span>{{ step + 1 }} / {{ guides.length }}</span><button v-if="step < guides.length - 1" class="primary" @click="setStep(step + 1)">다음 항목<ArrowRight :size="17" /></button><button v-else class="primary" @click="dialog = 'preview'">전체 글 확인<ArrowRight :size="17" /></button></footer>
        </main>

        <aside v-if="helpOpen" class="context-sidebar"><div class="context-heading"><span class="eyebrow">생각을 이어 가는 도움</span><BookOpen :size="19" /></div><h3>{{ kind === 'social' ? '해결책보다 먼저,\n문제의 조건을 보세요.' : '누구에게 어떤\n변화를 줄 수 있나요?' }}</h3><p>{{ guide.question }}</p><div class="context-divider" /><span class="eyebrow">이 항목에서 확인하기</span><ul class="context-checks"><li v-for="item in guide.checks" :key="item">{{ item }}</li></ul><div v-if="kind === 'business'" class="context-note"><strong>2026 예제의 린 캔버스</strong><p>아홉 항목이 서로 연결되는지 확인하세요. 인터뷰를 마치지 않아도 바로 작성할 수 있습니다.</p></div><div v-else class="context-note"><strong>상황이 바뀐다면</strong><p>같은 방법을 쓰더라도 입력, 조건, 판단 기준이 달라질 수 있습니다.</p><button class="text-button" @click="openVariation">조건 바꾸어 연습<ArrowRight :size="15" /></button></div><details class="context-sources"><summary>예제 자료의 출처</summary><p>{{ example.source }}</p><p>{{ project.source || '직접 작성하는 문제입니다.' }}</p></details><div class="storage-card"><ShieldCheck :size="19" /><p>이 기기에 저장됩니다.<small>다른 기기에서 이어 쓰려면 백업 파일을 내려받으세요.</small></p><button class="text-button" @click="downloadBackup(project)"><Download :size="15" />원고 백업</button></div></aside>
      </div>
  ...
  <div v-if="dialog" class="modal-backdrop" @click.self="dialog = ''"><section ref="dialogElement" role="dialog" aria-modal="true" aria-labelledby="dialog-title" class="modal" :class="{ wide: dialog === 'preview' || dialog === 'transfer' }" @keydown="trapDialog">
    ...
    <div class="modal-body">
      ...
      <template v-if="dialog === 'variation'"><p>변화 한 가지를 정하고, 내 원고의 어떤 부분을 고쳐야 할지 생각해 보세요. 새 사본에서 연습하므로 원래 글은 보존됩니다.</p><label class="field-label">달라진 조건<textarea v-model="variationText" rows="5" placeholder="예: 사용할 수 있던 통로 하나가 폐쇄되었습니다." /></label><button class="primary" :disabled="!variationText.trim()" @click="createVariation">사본에서 연습 시작<ArrowRight :size="16" /></button></template>
