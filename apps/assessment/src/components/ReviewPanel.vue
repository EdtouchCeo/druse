...
const emit = defineEmits<{ review: []; reason: [value: string]; check: [index: number, checked: boolean] }>()
...
<template>
  <section class="review-panel">
    ...
    <div v-if="review" class="review-content">
      ...
      <ul class="review-checks"><li v-for="(question, index) in review.prompts" :key="question"><label><input type="checkbox" :key="`${review.createdAt}-${index}`" :checked="review.checked?.includes(index) || false" @change="emit('check', index, ($event.target as HTMLInputElement).checked)" /><span>{{ question }}</span></label></li></ul>
